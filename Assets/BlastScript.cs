...
public class BlastScript : MonoBehaviour
{
    ...
    int curDelay;
    int curBlastDelay = 0;
    public int blastDelay;
    public GameObject lingeringBlast;
    public int blastLifespan;

    // Start is called before the first frame update
    void Start()
    {
        curDelay = fireDelay / 2;
        curBlastDelay = blastDelay / 2;
    }
    ...
    void Fire(enemyMovement target)
    {
        ...
        if (curBlastDelay >= blastDelay)
        {
            GameObject blast = Instantiate(lingeringBlast, target.transform.position, Quaternion.identity);
            Destroy(blast, blastLifespan);
            curBlastDelay = 0;
        }
    }
